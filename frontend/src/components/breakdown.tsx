...
const Breakdown = () => {
    ...
    const calculateTotalByCategory = (expenses: ExpenseInterface[]): Record<string,number> => {
        ...
        datasets: [
            {
                ...
                backgroundColor: Object.keys(categoryTotals).map((_, index) => `rgba(${index * 50}, ${index * 100}, 200, 0.2)`),
                borderColor: Object.keys(categoryTotals).map((_, index) => `rgba(${index * 50}, ${index * 100}, 200, 1)`), 
                borderWidth: 1,
                hoverOffset: 1         
            }
        ]

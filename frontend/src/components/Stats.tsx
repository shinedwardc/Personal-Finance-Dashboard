  ...
  useEffect(() => {
    if (data.length > 0){
        const selectedMonthExpenses = data.filter((expense) => {
          ...
          const expenseMonth = expense.date.slice(5,6) === "0" ? expense.date.slice(6,7) : expense.date.slice(5,7);
        ...
        let currYear = sortedCumulativeAmountPerBy.keys().next().value.slice(0,4);

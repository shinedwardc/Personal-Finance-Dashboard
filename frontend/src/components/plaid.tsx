...
const Plaid = () => {
    ...
    const createLinkToken = async () => {
      try {
        const response = await axios.post(
          "http://localhost:8000/create-link-token/",
        );
  ...
  const { open, ready } = usePlaidLink({
    ...
    onSuccess: async (publicToken) => {
      ...
      try {
        ...
        const response = await axios.post(
          "http://localhost:8000/exchange-public-token/",
          {
  ...
  return (
    <div className="card w-96 bg-base-100 shadow-2xl">
      <div className="mt-8">
        <figure>
          <img className="w-1/2" src={plaidLogo} alt="Plaid" />
        </figure>
      </div>
      <div className="card-body">
        ...
        <p>
          {localStorage.getItem("plaidAccessToken")
            ? "Your account is connected to Plaid."
            : "Connect your bank account to track expenses."}
        </p>
        <div className="card-actions justify-center">
          {!accessToken ? (
            ...
            <button className="btn btn-secondary" onClick={plaidDisconnect}>
              Disconnect
            </button>
          )}
        </div>
